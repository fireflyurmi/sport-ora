        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-md bg-white dark:bg-[#0D1527] p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-2xl transition-all scale-100 text-left">
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              This action cannot be undone. This will permanently cancel your slot reservation at the venue.
            </p>
            
            <div className="mt-6 flex items-center justify-end space-x-3">
              <button
                onClick={() => setIsOpen(false)}
                className="px-4 py-2 text-xs font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/60 rounded-xl transition-colors"
              >
                Go Back
              </button>
              <button
                onClick={handleCancelBooking}
                className="px-4 py-2 text-xs font-bold text-white bg-red-600 hover:bg-red-700 rounded-xl shadow-md shadow-red-600/10 transition-all active:scale-[0.98]"
              >
                Yes, Cancel Booking
              </button>
            </div>
